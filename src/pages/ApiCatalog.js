  ...
  const renderCategoryTree = (categories) => {
    ...
    const renderTreeItems = (items, level = 0) => {
      ...
      return items.map((category) => {
        ...
        // 如果没有API和子分类，不显示
        if (totalApiCount === 0 && !hasChildren) return null;
        
        // 树节点的标签内容（包含分类名称和API数量）
        const labelContent = (
          <Box display="flex" alignItems="center" width="100%">
            <span>{categoryName}</span>
            <span className={classes.categoryCount}>{totalApiCount}</span>
            
            {editMode && (
              <div className={classes.categoryActionsContainer}>
                <Tooltip title="编辑">
                  <IconButton 
                    size="small"
                    onClick={(e) => {
        ...
        return (
          ...
          >
            {/* 显示此分类下的API */}
            {categoryApis.length > 0 && categoryApis.map(api => (
              <TreeItem
                key={`api-${api.id}`}
                nodeId={`api-${api.id}`}
                label={
                  <Box display="flex" alignItems="center">
                    <HttpIcon fontSize="small" style={{ marginRight: 8 }} />
                    <span>{api.name}</span>
                    {hasLineageData(api.id) && (
                      <LineageIcon fontSize="small" style={{ marginLeft: 8, color: '#f57c00' }} />
                    )}
                  </Box>
                }
    ...
    return (
      <TreeView
        ...
        selected={selectedApi ? `api-${selectedApi.id}` : ''}
